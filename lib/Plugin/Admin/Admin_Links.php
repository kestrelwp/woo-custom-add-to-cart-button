...
namespace Barn2\WCB_Lib\Plugin\Admin;

use Barn2\WCB_Lib\Registerable,
    Barn2\WCB_Lib\Service,
    Barn2\WCB_Lib\Plugin\Plugin,
    Barn2\WCB_Lib\Util;
...
class Admin_Links implements Registerable, Service {
    ...
    public function add_settings_link( $links ) {
        ...
        \array_unshift( $links, \sprintf( '<a href="%1$s">%2$s</a>', \esc_url( $settings_url ), __( 'Settings', 'woo-custom-add-to-cart-button' ) ) );
        return $links;
    }
    ...
    public function add_documentation_link( $links, $file ) {
        ...
        $row_meta = array(
            'docs' => \sprintf(
                ...
                \esc_attr( \sprintf( __( 'View %s documentation', 'woo-custom-add-to-cart-button' ), $this->plugin->get_name() ) ),
                \esc_html__( 'Docs', 'woo-custom-add-to-cart-button' )
            )
        );
